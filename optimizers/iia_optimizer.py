...
def load_prompts_from_range(start: int, amount: int) -> list[str]:
    prompts = [line.strip() for line in (ROOT / "coco_prompts.txt").read_text(encoding="utf-8").splitlines() if line.strip()]
    return prompts[start:start+amount]
...
class IIAOptimizer:
    ...
    def optimize_coefficients_for_timestep(
        ...
        try:
            ...
            print(f"    Timestep {timestep_idx}: β={beta_opt:.6f}, MSE: {mse_before:.2f} → {mse_after:.2f} ({improvement:+.1f}%)")
