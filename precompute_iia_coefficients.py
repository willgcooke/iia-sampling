"""
Precompute IIA-DDIM Coefficients (Faithful to Paper)

Computes optimal β coefficients for text-to-image IIA-DDIM following Eq. (8):
    z_{i+1} = Φ^DDIM(zi, ti) + βi * ε̈θ(zi, P; ti)

The optimization averages over:
- Initial noise distribution
- Text prompt distribution (using diverse prompts)
"""

import torch
from diffusers import DiffusionPipeline, DDIMScheduler
from iia_optimizer import IIAOptimizer, DIVERSE_PROMPTS
import time
import os
...
def main():
    ...
    OUTPUT_PATH = "iia_coefficients.pt"
    CHECKPOINT_PATH = "iia_checkpoint.pt"
    
    print(f"Config: {NUM_SAMPLES} samples, {NUM_FINE_STEPS} fine-steps, batch {BATCH_SIZE}")
    print(f"Guidance scale: {GUIDANCE_SCALE}")
    print(f"Prompts: {len(DIVERSE_PROMPTS)} diverse prompts")
    ...
    save_data = {
        ...
        'latent_shape': latent_shape,
        'device': DEVICE,
        'num_prompts': len(DIVERSE_PROMPTS),
    }
