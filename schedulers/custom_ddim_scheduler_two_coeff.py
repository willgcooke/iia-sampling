...
class CustomDDIMSchedulerTwoCoeff(DDIMScheduler):
    @register_to_config
    def __init__(
        self,
    ...
    def load_iia_coefficients(self, num_steps: int, path: str = None):
        if path is None:
            ...
            path = str(COEFFICIENTS_DIR / f"iia_coefficients_two_coeff_{num_steps}.pt")
    ...
    def step(
        ...
        curr_eps = model_output
        curr_x0 = (sample - beta_prod_t.sqrt() * curr_eps) / alpha_prod_t.sqrt()
        ...
        if self.use_iia and self.iia_coefficients is not None:
            ...
            coeffs = self.iia_coefficients.get(timestep_key, {"phi0": 0.0, "phi1": 0.0})
            phi0 = float(coeffs.get("phi0", 0.0))
            phi1 = float(coeffs.get("phi1", 0.0))
            
            if self.prev_eps is not None and self.prev_x0 is not None:
                if phi0 != 0.0 or phi1 != 0.0:
                    delta_x0 = curr_x0 - self.prev_x0
                    delta_eps = curr_eps - self.prev_eps
                    correction = phi0 * delta_x0 + phi1 * delta_eps
                    prev_sample = prev_sample + correction
        
        self.prev_eps = curr_eps.clone()
        self.prev_x0 = curr_x0.clone()
